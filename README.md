...
若串口是 `Advertising started` 且 `SF=0`，家庭 App **不会**把它当成新配件。这是 NVS 里残留的 `pairing_list`。烧录后第一次启动应打印 `One-time clear of leftover HomeKit pairings` 并广播 `SF=1`。之后再配对会保留。仍显示已配对时：
